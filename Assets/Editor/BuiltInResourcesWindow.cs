...
public class BuiltInResourcesWindow : EditorWindow
{
    ...
    {
        var window = GetWindow<BuiltInResourcesWindow>();
        window.Show();
    }
    ...
    private void OnGUI()
    {
        ...
        if (_drawings == null)
        {
            ...
            if (_showingStyles)
            {
                foreach (GUIStyle customStyle in GUI.skin.customStyles)
                {
                    if (lowerSearch != "" && !customStyle.name.ToLower().Contains(lowerSearch))
                        continue;

                    GUIStyle thisStyle = customStyle;
                    float width = Mathf.Max(
                        100.0f,
                        GUI.skin.button.CalcSize( new GUIContent( customStyle.name ) ).x,
                        customStyle.CalcSize( inactiveText ).x + customStyle.CalcSize( activeText ).x
                                      ) + 16.0f;
            ...
            else if (_showingIcons)
            {
                ...
                foreach (UnityEngine.Object textureObject in _objects)
                {
                    var texture = (Texture)textureObject;
    ...
    void CopyText(string pText)
    {
        ...
        {
            //content = new GUIContent(pText) // Unity 4.x code
            text = pText // Unity 5.x code
        };
